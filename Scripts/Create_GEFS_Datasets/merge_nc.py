...
def load_nc(VarSName, file, VarFH, EM):
    '''
...
#                  str(VarFH) + '.nc'
    ...
    with Dataset(file, 'r') as nc:
        ...
        if file == './Data/tmp/GEFS_' + str(VarSName) + '_000-' + str(EM) + '.nc':
...
def load_multiple_nc(VarSName, FH, EM):
    '''
...
#    path = '/Users/Rarrell/Desktop/Thesis_Research/tmp/'
    ...
    with Dataset(path + 'GEFS_' + str(VarSName) + '_000-' + str(EM) + '.nc', 'r') as nc:
        tmp = nc.variables[str(VarSName)][:,:]
    
    # Collect dimensions
    T = len(glob(path + 'GEFS_' + str(VarSName) + '_*-' + str(EM) + '.nc'))
    J, I = tmp.shape
    ...
    for n, file in enumerate(glob(path + 'GFS_' + str(VarSName) + '_*.nc')):
        if file == path + 'GEFS_' + str(VarSName) + '_000-' + str(EM) + '.nc':
...
def write_nc(VarSName, VarName, EM, Var, VarFH, lat, lon, VarVD, mask, units,
             year, month, day, ModelRun, TypeOfHeight):
    '''
    ...
    filename = 'GEFS_' + str(VarSName) + '_' + str(year) + str(month) +\
               str(day) + '_' + str(ModelRun) + '_' + str(EM) + '.nc'
#    path = '/Users/Rarrell/Desktop/Thesis_Research/Data/'
    ...
    with Dataset(path + filename, 'w', format = 'NETCDF4') as nc:
        ...
        nc.description = 'GEFS forecast data for ' + VarName + ' valid for ' +\
                         day + '-' + month + '-' + year + ' and for ensemble member' +\
                         str(EM) + '. The variable is ' +\
