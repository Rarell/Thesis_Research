...
def main():
    ...
    VarData, lat, lon, VarUnits, VarFH, VarVD, VarName, VarSName, Mask =\
        grb_extract_data(var = Var, TypeOfHeight = TypeOfHeight, 
                         height = Height, year = Year, month = Month, day = Day, EM = EM,
    ...
    create_tmp_nc(VarData = VarData, lat = lat, lon = lon, VarUnits = VarUnits,
                  ...
                  VarSName = VarSName, EM = EM, Mask = Mask, TypeOfHeight = TypeOfHeight)
...
#################################
def grb_extract_data(var, TypeOfHeight, height, year, month, day, EM, 
    ...
    if int(request) == 1:
        filename = 'gens-b_3_' + str(year) + str(month) + str(day) + '_' +\
                   str(model_run) + '00_' + str(forecast_hour) + '_' + str(EM) + '.grb2'
        path = './Data/tmp/gens_3_' + str(year) + str(month) + str(day) +\
               str(model_run) +  '_' + str(EM) + '.g2/'
...
#        path = '/Users/Rarrell/Downloads/tmp/'
        ...
        if source == np.str('NCEI'):
            filename = 'gens-b_3_' + str(year) + str(month) + str(day) + '_' +\
                   str(model_run) + '00' + '_' + str(forecast_hour) + '_' + str(EM) + '.grb2'
        elif source == 'NCEP':
            filename  = 'gep' + str(EM) + '.t' + str(model_run) + 'z.pgrb2f' +\
                        str(forecast_hour)
        else:
    ...
    # Open the .grb2 file
    gefs = pygrib.open(grb_file)
    
    # Find the message for the variable
    VarMsg = gefs.select(name = var, typeOfLevel = TypeOfHeight, level = height)
    ...
    MaskMsg = gefs.select(name = 'Land-sea mask',
                      typeOfLevel = 'surface')
...
#    path = '/Users/Rarrell/Desktop/Thesis_Research/tmp/'
    ...
    with Dataset(path + filename, 'w', format = 'NETCDF4') as nc:
        ...
        nc.description = "This is a temporary file will hold GEFS data for ensemble member " +\
                         str(EM) + "for " + VarName + ". This will be for the " +\
