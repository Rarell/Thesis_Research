...
def main():
    ...
    Years, Months, Days = np.loadtxt(tmplist, usecols = (0, 1, 2), 
                                     delimiter = ' ', dtype = str, unpack = True)
    
    LE = load_nc(SNameLE, Years[-1], Months[-1], Days[-1])
    PET = load_nc(SNamePET, Years[-1], Months[-1], Days[-1])
    ...
    write_nc('sesr', 'SESR', SESR, LE['FH'], ESRM['lat1d'], ESRM['lon1d'],
             np.unique(LE['ymd']), Years[-1], Months[-1], Days[-1])
...
def write_nc(VarSName, VarName, Var, VarFH, lat, lon, dates, year, month, day):
    '''
    ...
    filename = 'GFS_' + str(VarSName) + '_ed_' + str(year) +\
               str(month) + str(day) + '.nc'
